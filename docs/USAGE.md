...
|---|---|---|---|
| `QuoteSnapshot` | `quoteReference` (or `externalReference` when no quote ref) | every Risk-domain ingest (Polaris RiskSubmission, QuoteForge QuoteRequest, BindPoint PolicyBindRequest) | `GET /api/v1/quotes/{quoteReference}` |
...
**Smaller-source case**: a Polaris-only ingest produces a thin `QuoteSnapshot` with insured / premium hint / quote status filled but no broker block, no bind data, no claims. As more events arrive (a QuoteForge quote with broker info, a BindPoint bind that creates the policy), the same snapshot is enriched in place. Same shape for everyone — just partially filled until more is known.
...
| `Source` | `POLARIS_UW`, `QUOTEFORGE`, `BINDPOINT`, ..., or `internal` for cancel / endorse / renew |
...
- `Version` — increments on each quote-issuance event (Polaris `RiskSubmission`, QuoteForge `QuoteRequest`, ...). Stays put on bind / cancel / endorse.
...
[
  {
    "systemCode": "POLARIS_UW",
    "displayName": "Polaris Underwriting Workbench",
...
| `source` | string | yes | e.g. `POLARIS_UW`, `CLAIMFORGE` |
...
|---|---|---|---|
| `POLARIS_UW` | `RiskSubmission` | `RiskIngestHandler` → `PolarisRiskMapper` | `PolarisRiskSubmissionPayload` |
...
**Request (Polaris risk submission)**
...
```json
{
  ...
  "self": "/api/v1/ingest/POLARIS_UW/evt-0001",
  "outcome": {
    "entityId": "…",
...
```json
{
  ...
  "externalReferences": { "POLARIS_UW": "Q-100045", "QUOTEFORGE": "QT-2201", "BINDPOINT": "POL-7781" },
  "history": [
