...
public sealed class PolarisRiskMapper : ISourceRiskMapper
{
    public bool CanMap(SourceIngestRequest request)
    {
        return string.Equals(request.SourceSystem, "POLARIS_UW", StringComparison.OrdinalIgnoreCase)
    ...
    {
        var payload = request.Payload.Deserialize<PolarisRiskSubmissionPayload>(new JsonSerializerOptions(JsonSerializerDefaults.Web));

        if (payload is null)
        {
            throw new InvalidOperationException("Unable to deserialize Polaris risk payload.");
        }
        ...
        return new CanonicalRiskRequest
        {
            ...
            CurrencyCode = "USD",
            UnderwriterName = "Polaris Intake",
